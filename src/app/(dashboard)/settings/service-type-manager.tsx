...
export function ServiceTypeManager({ initialData, title, description }: Props) {
  ...
  const handleUpdate = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        toast.success("?œë¹„???•íƒœ ?•ë³´ê°€ ?˜ì •?˜ì—ˆ?µë‹ˆ??");
        setEditingId(null);
      }
    });
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`'${name}' ?œë¹„???•íƒœë¥??? œ?˜ì‹œê² ìŠµ?ˆê¹Œ?\n???•íƒœë¥??¬ìš©?˜ëŠ” ê³ ê°?¬ê? ?ˆì„ ê²½ìš° ?? œê°€ ?¤íŒ¨?????ˆìŠµ?ˆë‹¤.`)) return;
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        toast.success("?œë¹„???•íƒœê°€ ?? œ?˜ì—ˆ?µë‹ˆ??");
      }
    });
  };
  ...
  const handleAdd = async () => {
    if (!newName.trim()) {
      toast.error("?œë¹„???•íƒœ ëª…ì¹­???…ë ¥?´ì£¼?¸ìš”.");
      return;
    }
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        toast.success("?ˆë¡œ???œë¹„???•íƒœê°€ ì¶”ê??˜ì—ˆ?µë‹ˆ??");
        setIsAddOpen(false);
        setNewName("");
      }
    });
    ...
    <div className="space-y-3">
        ...
        <Table className="text-xs">
          <TableHeader className="bg-slate-800">
            <TableRow className="hover:bg-transparent border-none">
              ...
              <TableHead className="font-black text-slate-300 border-r border-slate-700/50 h-10 text-[12px] uppercase tracking-wider px-4">?œë¹„???•íƒœ ëª…ì¹­</TableHead>
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>??{title.replace(" ê´€ë¦?, "")} ì¶”ê?</DialogTitle>
            <DialogDescription>
              ?œìŠ¤?œì— ?±ë¡???ˆë¡œ??{title.replace(" ê´€ë¦?, "")} ëª…ì¹­???…ë ¥?˜ì„¸??
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                placeholder={`${title.replace(" ê´€ë¦?, "")} ?´ë¦„ ?…ë ¥`}
              />
            </div>
          </div>
          <DialogFooter>
